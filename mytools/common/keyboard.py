...
class Keyboard:
    def  __init__(self):
        self.keys = {'VK_BACK': 0x08, 'VK_TAB': 0x09, 'VK_CLEAR': 0x0C, 'VK_RETURN': 0x0D, 'VK_ENTER': 0x0D, 'VK_SHIFT': 0x10,
                     'VK_CONTROL': 0x11,
                     ...
                     'VK_PA1': 0xFD,
                     'VK_OEM_CLEAR': 0xFE, }
        self.sorted_keys = list(self.keys.keys())
        self.sorted_keys.sort(key=len)

    def is_pressed(self, _key: str = None):
        if _key is not None:
            for x in self.sorted_keys:
                if _key.lower() in x.lower():
                    _key = x
                    break
            if win32api.GetKeyState(self.keys[_key]) < 0:
                return True
        else:
            for k in self.sorted_keys:
                if win32api.GetKeyState(self.keys[k]) < 0:
                    return True
        return False
    ...
    def press(self, key: str or list):
    ...
    def whichPressed(self):
        while True:
            for x in self.sorted_keys:
                if win32api.GetKeyState(self.keys[x]) < 0:
                    return x.removeprefix("VK_").lower().removeprefix("oem_")


if __name__ == '__main__':
    print(Keyboard().whichPressed())