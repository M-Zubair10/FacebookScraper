...
def merge(images, structure, spaceT=0, spaceB=0, spaceL=0, spaceR=0, spaceColor: tuple or int = 255, bbox=False):
    """
    Merge all images on single output image
    :param images: n images
    :param structure: (rows, columns)
    :param spaceL: border on left side of image
    :param spaceR: border on right side of image
    :param spaceT: border on top side of image
    :param spaceB: border on bottom side of image
    :param spaceColor: border color in BGR format
    :param bbox: bounding boxes of images on output image
    :return: image (bbox optional)
    """
    # Determine number of rows and columns
    rows, cols = structure
    cimages = copy.copy(images)
    cimages = [cv2.copyMakeBorder(img, spaceT, spaceB, spaceL, spaceR, cv2.BORDER_CONSTANT, value=spaceColor) for img in cimages]

    # Create empty canvas for output image
    canvas_height = rows * cimages[0].shape[0]
    canvas_width = cols * cimages[0].shape[1]
    canvas = np.zeros((canvas_height, canvas_width, 3), dtype=np.uint8)
    boxes = []

    # Resize input images to match output canvas dimensions
    for i in range(len(images)):
        cimages[i] = cv2.resize(cimages[i], cimages[0].shape[:2][::-1])

    # Loop over each cell in the structure
    for i in range(rows):
        for j in range(cols):
            # Compute the index of the image for this cell
            img_idx = i * cols + j
            ...
            if img_idx < len(images):
                ...
                # Compute the coordinates of the cell in the output image
                x1 = j * cimages[0].shape[1]
                y1 = i * cimages[0].shape[0]
                x2 = x1 + cimages[0].shape[1]
                y2 = y1 + cimages[0].shape[0]
                boxes.append([x1, y1, x2, y2])

                # Insert the image into the output image
