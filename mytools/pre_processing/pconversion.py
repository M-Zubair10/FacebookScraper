...
class dict2object:
    def __init__(self, dictionary):
        assert not [k for k in dictionary.keys() if not isinstance(k, str)], 'variable must be equal to str'
...
def path_to_base64(path, encoding="ascii"):
...
def cv2_to_base64(img: np.ndarray, encoding='ascii'):
...
def pil_to_base64(img, encoding='ascii'):
    b64 = cv2_to_base64(pil_to_cv2(img), encoding=encoding)
    return b64
...
def encode_unicode(string):
    """Encode the given string to unicode escape sequence"""
    result = ""
    for c in string:
        result += "\\u{0:04x}".format(ord(c))
    return result